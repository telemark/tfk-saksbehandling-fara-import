...
module.exports = miss.through(function (chunck, encoding, callback) {
  ...
  var options = {
    skoleid: parseInt(item.skoleData.id),
    postnummer: parseInt(item.dsfData.POSTN),
    gatenavn: item.dsfData.ADR.replace(/[0-9]/g, '').replace(/\s+$/, ''),
    husnummer: parseInt(item.dsfData.ADR.replace(/\D/g, ''))
  }
  console.log(options)
